...
def copy_all_python_files(source, snapshot_main_dir, code_snapshot_hash):
    ...
    Copies following files from source to destination:
        ...
        b) all multimodelity/*.py recursively.
    """
    ...
    all_pys = (
        glob(os.path.join(source, "multimodelity/**/*.py"), recursive=True)
...
def launch_train(args, config):
    ...
    train_cmd = ["python", "-u", os.path.join(destination, "multimodelity_cli/run.py")]
