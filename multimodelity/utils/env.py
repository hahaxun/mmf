...
def import_user_module(user_dir: str):
    ...
    if user_dir:
        ...
        sys.modules["multimodelity_user_dir"] = sys.modules[module_name]
        if module_bak is not None and module_name != "multimodelity_user_dir":
...
def import_files(file_path: str, module_name: str = None):
    ...
    ```
    from multimodelity.utils.env import import_files
...
def setup_imports():
    ...
    if root_folder is None:
        ...
        environment_multimodelity_path = os.environ.get("multimodelity_PATH", os.environ.get("PYTHIA_PATH"))
    ...
    for f in files:
        ...
        if f.endswith(".py") and not f.endswith("__init__.py"):
            ...
            module = ".".join(["multimodelity"] + splits[import_prefix_index:-1] + [module_name])
