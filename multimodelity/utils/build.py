...
import multimodelity
import torch
from multimodelity.common import typings as multimodelity_typings
...
from multimodelity.utils.distributed import is_dist_initialized
from multimodelity.utils.general import get_optimizer_parameters
...
def build_trainer(config: multimodelity_typings.DictConfig) -> Any:
...
def build_model(
    config: Union[DictConfig, "multimodelity.models.base_model.BaseModel.Config"]
) -> "multimodelity.models.base_model.BaseModel":
...
def build_dataset(
...
) -> multimodelity_typings.DatasetType:
    ...
    """
    from multimodelity.utils.configuration import load_yaml_with_defaults
    ...
    builder_instance: multimodelity_typings.DatasetBuilderType = dataset_builder()
...
def build_dataloader_and_sampler(
    dataset_instance: multimodelity_typings.DatasetType, training_config: multimodelity_typings.DictConfig
) -> multimodelity_typings.DataLoaderAndSampler:
    """Builds and returns a dataloader along with its sample

    Args:
        dataset_instance (multimodelity_typings.DatasetType): Instance of dataset for which
            dataloader has to be created
        training_config (multimodelity_typings.DictConfig): Training configuration; required
            for infering params for dataloader

    Returns:
        multimodelity_typings.DataLoaderAndSampler: Tuple of Dataloader and Sampler instance
    """
    from multimodelity.common.batch_collator import BatchCollator
...
def _add_extra_args_for_dataloader(
    dataset_instance: multimodelity_typings.DatasetType,
    other_args: multimodelity_typings.DataLoaderArgsType = None,
) -> multimodelity_typings.DataLoaderArgsType:
    from multimodelity.utils.general import get_batch_size
...
def build_classifier_layer(config, *args, **kwargs):
    from multimodelity.modules.layers import ClassifierLayer
...
def build_text_encoder(config, *args, **kwargs):
    try:
        from multimodelity.modules.fb.encoders import TextEncoderFactory
    except ImportError:
        from multimodelity.modules.encoders import TextEncoderFactory
...
def build_image_encoder(config, direct_features=False, **kwargs):
    from multimodelity.modules.encoders import ImageEncoderFactory, ImageFeatureEncoderFactory
...
def build_encoder(config: Union[DictConfig, "multimodelity.modules.encoders.Encoder.Config"]):
    from multimodelity.modules.encoders import Encoder
