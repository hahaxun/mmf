...
from multimodelity.utils.env import import_user_module
from multimodelity.utils.file_io import PathManager
from multimodelity.utils.general import get_absolute_path, get_multimodelity_root
...
def get_multimodelity_cache_dir():
    config = get_global_config()
    cache_dir = config.env.cache_dir
    # If cache_dir path exists do not join to multimodelity root
    if not os.path.exists(cache_dir):
        cache_dir = os.path.join(get_multimodelity_root(), cache_dir)
    return cache_dir


def get_multimodelity_env(key=None):
    config = get_global_config()
    if key:
...
def resolve_cache_dir(env_variable="multimodelity_CACHE_DIR", default="multimodelity"):
    ...
    if not PathManager.exists(cache_path):
        try:
            ...
            cache_path = os.path.join(get_multimodelity_root(), ".multimodelity_cache")
            PathManager.mkdirs(cache_path)
...
class Configuration:
    ...
    def import_user_dir(self):
        # Try user_dir options in order of multimodelity configuration hierarchy
