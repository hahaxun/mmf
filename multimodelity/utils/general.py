...
import torch
from multimodelity.utils.distributed import get_rank, get_world_size
...
def get_multimodelity_root():
    ...
    if multimodelity_root is None:
        ...
        multimodelity_root = os.path.abspath(os.path.join(multimodelity_root, ".."))
...
def get_absolute_path(paths):
    ...
    if isinstance(paths, str):
        ...
        from multimodelity.utils.configuration import get_multimodelity_env

        user_dir = get_multimodelity_env(key="user_dir")
        if user_dir:
            possible_paths.append(os.path.join(user_dir, paths))

        multimodelity_root = get_multimodelity_root()
        # Relative to root folder of multimodelity install
        possible_paths.append(os.path.join(multimodelity_root, "..", paths))
        # Relative to multimodelity root
        possible_paths.append(os.path.join(multimodelity_root, paths))
...
def get_batch_size():
    from multimodelity.utils.configuration import get_global_config
