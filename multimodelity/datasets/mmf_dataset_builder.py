...
from multimodelity.datasets.subset_dataset import multimodelitySubset
from multimodelity.utils.configuration import get_global_config, get_multimodelity_env, get_zoo_config
from multimodelity.utils.general import get_absolute_path
from omegaconf import open_dict


class multimodelityDatasetBuilder(BaseDatasetBuilder):
    ...
    def _download_requirement(
        ...
        download_path = os.path.join(
            get_multimodelity_env("data_dir"), "datasets", dataset_name, dataset_variation
        )
    ...
    def load(self, config, dataset_type, *args, **kwargs):
        ...
        dataset = multimodelityConcatDataset(datasets)
        if split_dataset_from_train:
    ...
    def _split_dataset_from_train(self, dataset, dataset_type):
        if dataset_type in self.config.split_train.keys() or dataset_type == "train":
            ...
            dataset = multimodelitySubset(dataset, indices)
            print(
