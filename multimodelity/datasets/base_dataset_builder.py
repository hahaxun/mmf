...
"""
In multimodelity, for adding new datasets, dataset builder for datasets need to be
...
to add dataset builder to ease setup of multimodelity.

``load`` is used to load a dataset from specific path. ``load`` needs to return
an instance of subclass of ``multimodelity.datasets.base_dataset.BaseDataset``.
...
.. _here: https://github.com/facebookresearch/multimodelity/blob/master/multimodelity/datasets/vqa/vqa2/builder.py
"""
...
class BaseDatasetBuilder:
    ...
    def load_dataset(self, config, dataset_type="train", *args, **kwargs):
        """Main load function use by multimodelity. This will internally call ``load``
