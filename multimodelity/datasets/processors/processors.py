...
"""
The processors exist in multimodelity to make data processing pipelines in various
...
in multimodelity, processor also accept a ``DictConfig`` with a `type` and `params`
...
import torch
from multimodelity.common.registry import registry
from multimodelity.common.typings import ProcessorConfigType
from multimodelity.utils.configuration import get_multimodelity_cache_dir, get_multimodelity_env
from multimodelity.utils.distributed import is_master, synchronize
from multimodelity.utils.file_io import PathManager
from multimodelity.utils.text import VocabDict
from multimodelity.utils.vocab import Vocab, WordToVectorDict
...
class BaseProcessor:
    """Every processor in multimodelity needs to inherit this class for compatibility
    with multimodelity. End user mainly needs to implement ``__call__`` function.
...
class Processor:
    """Wrapper class used by multimodelity to initialized processor based on their
...
class FastTextProcessor(VocabProcessor):
    ...
    def _try_download(self):
        ...
        if not PathManager.exists(model_file):
            model_file = os.path.join(get_multimodelity_cache_dir(), model_file)
    ...
    def _download_model(self):
        ...
        model_file_path = os.path.join(get_multimodelity_cache_dir(), "wiki.en.bin")
...
class SimpleSentenceProcessor(BaseProcessor):
    ...
    def __init__(self, *args, **kwargs):
        from multimodelity.utils.text import tokenize
...
class BBoxProcessor(VocabProcessor):
    ...
    def __init__(self, config, *args, **kwargs):
        from multimodelity.utils.dataset import build_bbox_tensors
...
class PhocProcessor(VocabProcessor):
    ...
    def __init__(self, config, *args, **kwargs):
        from multimodelity.utils.phoc import build_phoc
