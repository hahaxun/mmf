...
class ImageDatabase(torch.utils.data.Dataset):
    """ImageDatabase can be used to load images in multimodelity.
    This goes either in conjunction with AnnotationDatabase or
    can be separately used with function such as `from_path`.
    multimodelityDataset initializes its own copy of ImageDatabase if `use_images`
    ...
    of image database or free to update or ignore multimodelityDataset's ImageDataset
